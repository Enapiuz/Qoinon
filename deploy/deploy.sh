...
ssh -oStrictHostKeyChecking=no root@188.166.114.242 /bin/bash << EOF
    ...
    source ../../ucenv/bin/activate && sudo -u enapiuz python manage.py migrate --noinput
    source ../../ucenv/bin/activate && sudo -u enapiuz python manage.py bower install
    source ../../ucenv/bin/activate && sudo -u enapiuz python manage.py collectstatic --noinput
    cd ../
